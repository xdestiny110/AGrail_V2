...
namespace Framework.UI
{
    public sealed class UIFactory : Singleton<UIFactory>
    {
        ...
        public GameObject CreateWindows(string uiName, bool isResource = false)
        {
            if (!goPool.ContainsKey(uiName))
            {
                if (!isResource)
                    goPool.Add(uiName, AssetBundleManager.Instance.LoadAsset<GameObject>(UIPrefabPath, uiName));
                else
                    goPool.Add(uiName, Resources.Load<GameObject>(UIPrefabPath + "/" + uiName));
            }
            return Object.Instantiate(goPool[uiName]);
        }

        public IEnumerator CreateWindowsAnyn(string uiName,  bool isResource = false)
        {
            if (!goPool.ContainsKey(uiName))
            {
                if (!isResource)
